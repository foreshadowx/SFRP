...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, group, \
    FRPS, HybirdRear
import copy
...
class SFRP(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = channels[-1] +channels[-2] +channels[-3] +channels[-4]
        self.channel_list = channels

        self.ds1 = FRPS(512, 32, 32)
        self.ds2 = FRPS(256, 64, 32)
        self.ds3 = FRPS(128, 128, 32)
        self.ds4 = FRPS(64, 256, 32)

        self.rear1 = HybirdRear(64)
        self.rear2 = HybirdRear(128)
        self.rear3 = HybirdRear(256)
        self.rear4 = HybirdRear(512)
    ...
    def forward_cls_feat(self, p0,f0=None):
        ...
        for i in range(0, len(self.encoder)):
            pe = None
            if i == 1:
                ds_xyz, ds_attention = self.ds1(p0.permute(0, 2, 1), f0)
            p0, f0, pe = self.encoder[i]([p0, f0, pe])
            if i == 1:
                ds_attention = self.conv1(ds_attention)
                x_attention_all = torch.cat([f0, ds_attention], dim=-1)
                x_attention_all = self.rear1(x_attention_all)
                self.res_link_list.append(x_attention_all.max(dim=-1)[0])
                f0 = x_attention_all[..., :512]
            elif i == 2:
                ds_xyz, ds_attention = self.ds2(ds_xyz, x_attention_all[..., 512:])
                ds_attention = self.conv2(ds_attention)
                x_attention_all = torch.cat([f0, ds_attention], dim=-1)
                x_attention_all = self.rear2(x_attention_all)
                self.res_link_list.append(x_attention_all.max(dim=-1)[0])
                f0 = x_attention_all[..., :256]
            elif i == 3:
                ds_xyz, ds_attention = self.ds3(ds_xyz, x_attention_all[..., 256:])
                ds_attention = self.conv3(ds_attention)
                x_attention_all = torch.cat([f0, ds_attention], dim=-1)
                x_attention_all = self.rear3(x_attention_all)
                self.res_link_list.append(x_attention_all.max(dim=-1)[0])
                f0 = x_attention_all[..., :128]
            elif i == 4:
                ds_xyz, ds_attention = self.ds4(ds_xyz, x_attention_all[..., 128:])
                ds_attention = self.conv4(ds_attention)
                x_attention_all = torch.cat([f0, ds_attention], dim=-1)
                x_attention_all = self.rear4(x_attention_all)
                self.res_link_list.append(x_attention_all.max(dim=-1)[0])

        return pack(self.res_link_list, 'B *')[0]
